...
int prevNavballX[MAX_NAVBALL_POINTS];
int prevNavballY[MAX_NAVBALL_POINTS];
int prevNavballCount = 0;

int project(float x, float y, 
            float z, int *sx, 
            int *sy) 
{
	if (z < 0) return 0; // Don't draw behind the viewer
...
{
	// Yaw (around Z)
	float tx = fcos(yaw) * *x - fsin(yaw) * *y;
	float ty = fsin(yaw) * *x + fcos(yaw) * *y;
	*x = tx; *y = ty;

	// Pitch (around X)
	float tz = fcos(pitch) * *z - fsin(pitch) * *y;
	ty = fsin(pitch) * *z + fcos(pitch) * *y;
	*z = tz; *y = ty;

	// Roll (around Y)
	tx = fcos(roll) * *x + fsin(roll) * *z;
	tz = -fsin(roll) * *x + fcos(roll) * *z;
	*x = tx; *z = tz;
}
                 ...
                 float yawDeg) 
{
	...
	// Outer navball circle
	st7735s_draw_circle(lcd, radius, cx, cy, COLOR565_GRAY);

	for (int i = 0; i < prevNavballCount; i++) {
		st7735s_draw_pixel(lcd, prevNavballX[i], prevNavballY[i], COLOR565_BLACK);
	}
	prevNavballCount = 0;

	// Dot field (points on a hemisphere)
	for (float theta = -PI/2; theta <= PI/2; theta += PI/12) {        // vertical angle
		for (float phi = 0; phi < 2*PI; phi += PI/18) {                 // horizontal angle
			...
			// World horizon = y == 0; ground is y < 0
			if (y >= 0) continue;

			// Apply orientation
			rotate(&x, &y, &z, pitchRAD, rollRAD, yawRAD);

			int sx, sy;
			if (project(x, y, z, &sx, &sy)) {
				if ((sx - cx)*(sx - cx) + (sy - cy)*(sy - cy) <= radius*radius){
					st7735s_draw_pixel(lcd, sx, sy, COLOR565_GRAY);
					if (prevNavballCount < MAX_NAVBALL_POINTS) {
						prevNavballX[prevNavballCount] = sx;
						prevNavballY[prevNavballCount] = sy;
						prevNavballCount++;
					}
				}
			}
	...
	for (float theta = -PI/2; theta <= 2 * PI; theta += PI/90) {
		//float x = 0;
		//float y = sin(theta);
		//float z = cos(theta);
		float x = fcos(theta) * fcos(0);
		float y = fsin(theta);
		float z = fcos(theta) * fsin(0);
	...
	// Horizon ring (y == 0 on the globe) ON STM32?
	for (float phi = 0; phi < 2 * PI; phi += PI / 90) {
		float x = fcos(phi);
		float y = 0;
		float z = fsin(phi);
	...
	// Left slanting line (from left upward to center)
	st7735s_draw_line(lcd, cx - 5, cy + 5, cx, cy, COLOR565_GRAY);
	// Right slanting line (from center to right upward)
	st7735s_draw_line(lcd, cx, cy, cx + 5, cy + 5, COLOR565_GRAY);
	// Left slanting line
	st7735s_draw_line(lcd, cx - 5, cy + 5, cx - 10, cy, COLOR565_GRAY);
	// Right slanting line
	st7735s_draw_line(lcd, cx + 5, cy + 5, cx + 10, cy, COLOR565_GRAY);
	// Left line
	st7735s_draw_line(lcd, cx - 8, cy, cx - 18, cy, COLOR565_GRAY);
	// Right line
	st7735s_draw_line(lcd, cx + 8, cy, cx + 18, cy, COLOR565_GRAY);
}
